...
import { CachedKnowledgeGraphManager } from '../src/graph/CachedKnowledgeGraphManager.js';
import { EnhancedKnowledgeGraphManager } from '../src/graph/EnhancedKnowledgeGraphManager.js';
import { promises as fs } from 'fs';
import { join } from 'path';
import { tmpdir } from 'os';

class SimpleBenchmark {
  private results: Array<Record<string, string | number>> = [];

  async runBenchmarks() {
    console.log('🚀 Starting Performance Benchmarks...\n');

    const testFile = join(tmpdir(), `benchmark-${Date.now()}.jsonl`);
  ...
  async benchmarkCachingPerformance(testFile: string) {
